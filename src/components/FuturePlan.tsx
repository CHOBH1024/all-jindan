...
export function FuturePlan({ results, isPremium, onShowPremium }: Props) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: 'var(--hint)', margin: '0 0 20px' }}>진단 결과를 바탕으로, 앞으로의 삶을 만들어갑니다.</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 16 }}>
        {/* 목표 설정 */}
        <div style={{ background: 'rgba(255,253,248,0.9)', border: '1px solid var(--border)', borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <input
              ...
              placeholder="새 목표 입력..."
              style={{ flex: 1, padding: '9px 14px', borderRadius: 10, background: 'var(--card3)', border: '1px solid var(--border2)', color: 'var(--text-strong)', fontSize: 13 }}
            />
            <select
              value={goalPeriod}
              onChange={e => setGoalPeriod(e.target.value as 'short' | 'long')}
              style={{ padding: '9px', borderRadius: 10, background: 'var(--card3)', border: '1px solid var(--border2)', color: 'var(--sub)', fontSize: 12 }}
            >
              <option value="short">단기</option>
              <option value="long">장기</option>
            </select>
            <button onClick={addGoal} style={{ padding: '9px 16px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 800, color: 'var(--bg)', background: 'var(--text)' }}>추가</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {goals.map(g => (
              <div key={g.id} style={{ padding: '10px 12px', background: 'var(--card3)', borderRadius: 10, border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <button onClick={() => toggleGoal(g.id)} style={{ width: 20, height: 20, borderRadius: 6, border: '1px solid #475569', cursor: 'pointer', background: g.done ? 'var(--success)' : 'transparent', color: 'var(--bg)', fontSize: 11 }}>{g.done ? '✓' : ''}</button>
                  <span style={{ flex: 1, fontSize: 13, textDecoration: g.done ? 'line-through' : 'none', color: g.done ? 'var(--hint)' : 'var(--text-strong)' }}>{g.text}</span>
                  <span style={{ fontSize: 10, color: 'var(--sub)', fontWeight: 700 }}>{g.period === 'short' ? '단기' : '장기'}</span>
                </div>
                {/* 목표-진단-습관 연결 */}
                {goalLinks(g.text, results, habits).map((link, li) => (
                  <div key={li} style={{ fontSize: 10, color: 'var(--sub)', marginTop: 6, paddingLeft: 30 }}>
                    {link}
                  </div>
                ))}
              </div>
            ))}
            {goals.length === 0 && <div style={{ fontSize: 12, color: 'var(--hint)', textAlign: 'center', padding: 16 }}>목표를 추가해보세요</div>}
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(255,253,248,0.9)', border: '1px solid var(--border)', borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              style={{ flex: 1, padding: '9px 14px', borderRadius: 6, background: 'var(--card)', border: '1px solid var(--border2)', color: 'var(--text)', fontSize: 13 }}
            />
            <button onClick={addHabit} style={{ padding: '9px 16px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 800, color: 'var(--bg)', background: 'var(--text)' }}>추가</button>
            <button
              onClick={useShield}
              disabled={shieldUsed}
              ...
              style={{
                ...
                color: shieldUsed ? 'var(--hint)' : 'var(--accent)', whiteSpace: 'nowrap',
              }}
            >
            ...
            <button
              ...
              title="주간 리셋"
              style={{ padding: '9px 14px', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: 'var(--card)', border: '1px solid var(--border2)', color: 'var(--sub2)', whiteSpace: 'nowrap' }}
            >
              🔄 주간 리셋
            </button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {habits.map(h => (
              <div key={h.id} style={{ padding: '10px 12px', background: 'var(--card3)', borderRadius: 10, border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  ...
                  <span style={{
                    ...
                    color: h.days.length >= 4 ? 'var(--error)' : 'var(--success)',
                  }}>
                    🔥 이번 주 {h.days.length}일
                  </span>
                  <button onClick={() => removeHabit(h.id)} style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, cursor: 'pointer', background: 'rgba(248,113,113,0.12)', border: '1px solid rgba(248,113,113,0.4)', color: 'var(--error)' }}>삭제</button>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  {WEEK.map(d => (
                    <button
                      ...
                      style={{
                        width: 30, height: 30, borderRadius: 8, fontSize: 11, cursor: 'pointer', border: '1px solid var(--border2)',
                        background: h.days.includes(d) ? 'var(--success)' : 'transparent', color: h.days.includes(d) ? 'var(--bg)' : 'var(--hint)',
                        fontWeight: 700,
                      }}
                    >
                      {d}
            ...
            {habits.length === 0 && <div style={{ fontSize: 12, color: 'var(--hint)', textAlign: 'center', padding: 16 }}>습관을 추가하고 매일 체크하세요</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(138,109,59,0.25)', borderRadius: 16, padding: 20, marginTop: 16 }}>
        <h2 style={{ fontSize: 15, fontWeight: 800, margin: '0 0 10px', color: 'var(--accent)' }}>✨ 진단 기반 추천 목표</h2>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {suggestedGoals.map((g, i) => (
            <span key={i} style={{ padding: '8px 14px', borderRadius: 999, background: 'var(--accent-soft)', border: '1px solid var(--accent-border)', color: 'var(--sub)', fontSize: 12, fontWeight: 600 }}>{g}</span>
          ))}
        </div>
      </div>
    </div>
...
function QuarterMap({ goals, results, habits }: {
  ...
  return (
    <div style={{
      background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 20, marginBottom: 16,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 800, padding: '3px 10px', borderRadius: 999, background: 'rgba(138,109,59,0.08)', border: '1px solid var(--accent-border)', color: 'var(--accent)' }}>
          {doneCount}/{items.length}
        </span>
      </div>
      <div style={{ fontSize: 11, color: 'var(--sub2)', marginBottom: 10 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {items.map(item => (
          <div key={item.id} style={{
            ...
            background: item.auto ? 'rgba(138,109,59,0.08)' : 'var(--card2)',
            border: '1px solid ' + (item.auto ? 'rgba(138,109,59,0.3)' : 'var(--border)'),
          }}>
            <span style={{
              width: 20, height: 20, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 11, fontWeight: 800,
              background: item.auto ? 'var(--accent)' : 'transparent', border: '1.5px solid ' + (item.auto ? 'var(--accent)' : 'var(--border3)'),
              color: 'var(--bg)', flexShrink: 0,
            }}>
              {item.auto ? '✓' : ''}
            </span>
            <span style={{ fontSize: 12, color: item.auto ? 'var(--body-text)' : 'var(--text-strong)' }}>{item.label}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        return (
          <button
            ...
            style={{
              ...
              background: isDone ? 'rgba(138,109,59,0.08)' : 'var(--card2)',
              border: '1px solid ' + (isDone ? 'rgba(138,109,59,0.3)' : 'var(--border)'),
            }}
          >
            <span style={{
              width: 22, height: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 12, fontWeight: 800,
              background: isDone ? 'var(--accent)' : 'transparent', border: '1.5px solid ' + (isDone ? 'var(--accent)' : 'var(--border3)'),
              color: 'var(--bg)', flexShrink: 0,
            }}>
              {isDone ? '✓' : ''}
            </span>
            <span style={{ fontSize: 13, color: isDone ? 'var(--body-text)' : 'var(--text-strong)' }}>{m.label}</span>
          </button>
        );
      })}
    </div>
...
function WeeklyReport({ results, isPremium, onShowPremium }: { results: DiagnosisRecord[]; isPremium: boolean; onShowPremium: () => void }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid var(--accent-border)', borderRadius: 16, padding: 20, marginBottom: 16,
    }}>
      <h2 style={{ fontSize: 15, fontWeight: 800, margin: '0 0 8px' }}>📬 이번 주 리포트</h2>
      {loading ? (
        <div style={{ fontSize: 12, color: 'var(--hint)' }}>로딩 중...</div>
        ...
        <div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 8 }}>
            {[
              ...
              <div key={s.label} style={{ flex: 1, minWidth: 60, textAlign: 'center', background: 'rgba(15,23,42,0.5)', borderRadius: 10, padding: '10px 6px' }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--hint)' }}>{s.label}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize: 12, color: 'var(--sub)' }}>{report.message}</div>
          {/* 주간 코치 브리핑 */}
          {coach && (
            <div style={{ marginTop: 12, padding: '14px 16px', background: 'var(--card2)', border: '1px solid var(--border)', borderRadius: 10 }}>
              <div style={{ fontSize: 11, letterSpacing: 1.5, fontWeight: 800, color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 8 }}>
                🧑‍🏫 이번 주 코치 브리핑
              </div>
              <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 1.7, marginBottom: 8 }}>{coach.interpretation}</div>
              {isPremium ? (
                <div style={{ fontSize: 12, color: 'var(--body-text)', lineHeight: 1.9 }}>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-strong)', lineHeight: 1.8, fontFamily: "'Noto Serif KR',serif" }}>
              "이번 주, 관계에서 가장 에너지를 쓴 순간은 언제였나요?"
            </div>
            <div style={{ fontSize: 11, color: 'var(--sub2)', marginTop: 4 }}>주간 리포트는 개인 기록으로 저장됩니다.</div>
          </div>
          {/* 최근 진단 기반 미니 팁 */}
          {results[0] && (
            <div style={{ marginTop: 12, padding: '12px 14px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8 }}>
              <div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 800, color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 6 }}>
                🧩 최근 진단 기준 미니 팁
              </div>
              <div style={{ fontSize: 12, color: 'var(--body-text)', lineHeight: 1.7 }}>
                "{results[0].title}"에서 <strong style={{ color: 'var(--accent)' }}>{results[0].result}</strong> 결과를 기록했어요.{' '}
        ...
        <div style={{ fontSize: 12, color: 'var(--hint)', lineHeight: 1.7 }}>
          {weekResults.length > 0
            ? <>이번 주 진단 <strong style={{ color: 'var(--accent)' }}>{weekResults.length}개</strong> 기록! 꾸준함이 힘입니다 💪 <br />로그인하면 상세 리포트를 받을 수 있어요.</>
